...
/// PipelineOwner用于管理渲染树的根节点，它负责协调渲染流程和处理布局、绘制等任务。
///
...
class PipelineOwner with DiagnosticableTreeMixin {
  ...
  /// 将此 [PipelineOwner] 标记为链接到给定的 [PipelineManifold]。
  /// 通常，这仅在 root [PipelineOwner] 上直接调用。
  /// 当调用 [adoptChild] 时，子级会自动链接到其父级的 [PipelineManifold]。
...
/// 管理 [PipelineOwner] 树。
///
/// 所有的 [PipelineOwner] 都附加到同一个 [PipelineManifold] 上。
/// 这个 [PipelineManifold] 提供了一些共享功能，比如请求视觉更新（通过调用 [requestVisualUpdate]）。
/// 因此，[PipelineManifold] 为 [PipelineOwner] 提供了通常由绑定提供的功能，
/// 而不将 [PipelineOwner] 与特定的绑定实现绑定在一起。
///
/// [PipelineOwner] tree 的 root 通过将 manifold
/// 传递给 [PipelineOwner.attach]来连接到[PipelineManifold].
/// 当子节点通过 [PipelineOwner.adoptChild] 被采纳时，它们也会连接到同一个 [PipelineManifold] 上。
///
/// [PipelineOwner]s 可以在 [PipelineManifold] 上注册监听器，
/// 以便在 [PipelineManifold] 提供的某些值发生变化时得到通知。
