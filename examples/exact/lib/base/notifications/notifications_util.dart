...
class NotificationUtils {
  ...
  static Future<bool> requestBasicPermissionToSendNotifications() async {
    bool isAllowed = await AwesomeNotifications().isNotificationAllowed();
    if (!isAllowed) {
      await Get.dialog(AlertDialog(
        backgroundColor: const Color(0xfffbfbfb),
        title: const Text('Get Notified!',
            maxLines: 2, textAlign: TextAlign.center, style: TextStyle(fontSize: 22.0, fontWeight: FontWeight.w600)),
        content: Column(
  ...
  static Future<void> requestFullScheduleChannelPermissions(
      BuildContext context, List<NotificationPermission> requestedPermissions) async {
  ...
  static Future<List<NotificationPermission>> requestUserPermissions(BuildContext context,
    ...
    if (!await requestBasicPermissionToSendNotifications()) return [];

    // Check which of the permissions you need are allowed at this time
    List<NotificationPermission> permissionsAllowed =
        await AwesomeNotifications().checkPermissionList(channelKey: channelKey, permissions: permissionList);
    ...
    if (lockedPermissions.isEmpty) {
      ...
      await AwesomeNotifications()
          .requestPermissionToSendNotifications(channelKey: channelKey, permissions: permissionsNeeded);

      // After the user come back, check if the permissions has successfully enabled
      permissionsAllowed =
          await AwesomeNotifications().checkPermissionList(channelKey: channelKey, permissions: permissionsNeeded);
    } else {
                ...
                actions: [
                  ...
                  TextButton(
                    onPressed: () async {
                      ...
                      await AwesomeNotifications()
                          .requestPermissionToSendNotifications(channelKey: channelKey, permissions: lockedPermissions);

                      // After the user come back, check if the permissions has successfully enabled
                      permissionsAllowed = await AwesomeNotifications()
                          .checkPermissionList(channelKey: channelKey, permissions: lockedPermissions);
  ...
  static Future<bool> requestCriticalAlertsPermission(BuildContext context) async {
    ...
    List<NotificationPermission> permissionsAllowed =
        await requestUserPermissions(context, channelKey: null, permissionList: requestedPermissions);
    ...
    try {
      bool success = await AwesomeNotifications().createNotification(
          content: NotificationContent(
              id: id, channelKey: 'basic_channel', title: 'Simple Notification', body: 'Simple body'));
  ...
  ************************************************ */
  ...
  static Future<void> showNotificationWithTimeout(int id) async {
    await AwesomeNotifications().createNotification(content: NotificationContent(
      ...
      NotificationActionButton(key: 'AGREED1', label: 'I agree', autoDismissible: true),
      NotificationActionButton(key: 'AGREED2', label: 'I agree too', autoDismissible: true),
    ]);
  }
    ...
    await AwesomeNotifications().createNotification(
        ...
        localizations: {
          ...
          'es': NotificationLocalization(
              ...
              buttonLabels: {'AGREED1': 'Estoy de acuerdo', 'AGREED2': 'También estoy de acuerdo'}),
        });
  }
  ...
  ************************************************ */
  ...
  static Future<void> showNotificationWithActionButtons(int id) async {
    await AwesomeNotifications().createNotification(
        ...
        actionButtons: [
          ...
          NotificationActionButton(key: 'PROFILE', label: 'Profile', autoDismissible: true, enabled: false)
        ]);
  }
  ...
  static Future<void> showNotificationWithIconsAndActionButtons(int id) async {
    await AwesomeNotifications().createNotification(
        ...
        actionButtons: [
          NotificationActionButton(key: 'READ', label: 'Mark as read', autoDismissible: true),
          NotificationActionButton(key: 'PROFILE', label: 'Profile', autoDismissible: true, color: Colors.green),
          NotificationActionButton(
              key: 'DISMISS', label: 'Dismiss', isDangerousOption: true, actionType: ActionType.DismissAction)
        ]);
  }
  ...
  static Future<void> showNotificationWithActionButtonsAndReply(int id) async {
    await AwesomeNotifications().createNotification(
        content: NotificationContent(
            ...
            payload: {'uuid': 'user-profile-uuid'}),
        actionButtons: [
          NotificationActionButton(key: 'REPLY', label: 'Reply', autoDismissible: true, requireInputText: true),
          NotificationActionButton(key: 'READ', label: 'Mark as read', autoDismissible: true),
  ...
  static Future<void> showCallNotification(int id, int timeToWait) async {
    ...
    await AwesomeNotifications().createNotification(
        ...
        content: NotificationContent(
            ...
            autoDismissible: false,
            backgroundColor: (platformVersion == 'Android-31') ? const Color(0xFF00796a) : Colors.white,
  ...
  static Future<void> showAlarmNotification({required int id, int secondsToWait = 30, int snoozeSeconds = 30}) async {
    await AwesomeNotifications().createNotification(
        ...
        schedule: (secondsToWait < 5)
            ? null
            : NotificationCalendar.fromDate(date: DateTime.now().add(Duration(seconds: secondsToWait))));
  }
  ...
  ************************************************ */
  ...
  static Future<void> startForegroundServiceNotification(int id) async {
    await AndroidForegroundService.startForeground(
        ...
        actionButtons: [NotificationActionButton(key: 'SHOW_SERVICE_DETAILS', label: 'Show details')]);
  }
  ...
  ************************************************ */
  ...
  static Future<void> showBigPictureNotificationActionButtons(int id) async {
    await AwesomeNotifications().createNotification(
        ...
        actionButtons: [
          NotificationActionButton(key: 'READ', label: 'Mark as read', autoDismissible: true),
          NotificationActionButton(key: 'REMEMBER', label: 'Remember-me later', autoDismissible: false)
        ]);
  }
  ...
  ************************************************ */
  ...
  static Future<void> showGroupedNotifications(String channelKey) async {
    await AwesomeNotifications().createNotification(
        content: NotificationContent(
            id: 1, channelKey: channelKey, title: 'Little Jhonny', body: 'Hey dude! Look what i found!'));

    await Future.delayed(const Duration(seconds: 2));

    await AwesomeNotifications().createNotification(
        content: NotificationContent(id: 2, channelKey: 'grouped', title: 'Cyclano', body: 'What?'));

    await Future.delayed(const Duration(seconds: 2));

    await AwesomeNotifications().createNotification(
        content: NotificationContent(
            id: 3, channelKey: channelKey, title: 'Little Jhonny', body: 'This push notifications plugin is amazing!'));

    await Future.delayed(const Duration(seconds: 2));

    await AwesomeNotifications().createNotification(
        content: NotificationContent(id: 4, channelKey: channelKey, title: 'Little Jhonny', body: 'Its perfect!'));
  ...
  ************************************************ */
  ...
  static Future<DateTime?> pickScheduleDate(BuildContext context, {required bool isUtc}) async {
    TimeOfDay? timeOfDay;
  ...
  static Future<void> getNextValidMonday(BuildContext context) async {
    ...
    if (nextValidDate == null) {
      ...
      response = utils.AwesomeDateUtils.parseDateToString(nextValidDate.toUtc(), format: 'dd/MM/yyyy')!;
    }
  ...
  static Future<void> repeatMultiple5Crontab() async {
    ...
    await AwesomeNotifications().createNotification(
        ...
        schedule: NotificationAndroidCrontab(
            ...
            expirationDateTime: nowDate.copyWith().add(const Duration(seconds: 10, minutes: 1)).toUtc(),
            crontabExpression: '/5 * * * * ? *',
  ...
  static Future<void> showProgressNotification(int id) async {
    int maxStep = 10;
    int fragmentation = 4;
    for (var simulatedStep = 1; simulatedStep <= maxStep * fragmentation + 1; simulatedStep++) {
      ...
      udpateNotificationAfter1Second = Timer(const Duration(seconds: 1), () {
        _updateCurrentProgressBar(id: id, simulatedStep: currentStep, maxStep: maxStep * fragmentation);
